...
class TransformerDecoderLayerKVDangle(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        #use temperature for encoder-decoder attention:
        if getattr(args, "use_cross_temp_scheduler", False):
            self.encoder_attn.register_buffer("temperature", torch.FloatTensor([1]))
    ...
    def build_encoder_attention(self, embed_dim, args):
        if getattr(args, "relu_cross_atten", False) :
            return MultiheadAttentionRelu(
                embed_dim,
                args.decoder_attention_heads,
                kdim=getattr(args, "encoder_embed_dim", None),
                vdim=getattr(args, "encoder_embed_dim", None),
    ...
    def forward(
        self,
        x,
        encoder_out_value: Optional[torch.Tensor] = None,
        encoder_out_key: Optional[torch.Tensor] = None,
        encoder_padding_mask: Optional[torch.Tensor] = None,
        incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
        prev_self_attn_state: Optional[List[torch.Tensor]] = None,
        prev_attn_state: Optional[List[torch.Tensor]] = None,
        ...
        if self.encoder_attn is not None:
            ...
            if prev_attn_state is not None:
                ...
                assert incremental_state is not None
                self.encoder_attn._set_input_buffer(incremental_state, saved_state)
            # print("2 layer x : "+str(x[:1]))

            x, attn = self.encoder_attn(
                query=x,
                key=encoder_out_key,
                value=encoder_out_value,
                key_padding_mask=encoder_padding_mask,
                incremental_state=incremental_state,
                static_kv=True,
                need_weights=need_attn or (not self.training and self.need_attn),
        ...
        # print("4 layer x : "+str(x[:1]))

        x = self.activation_fn(self.fc1(x))

        # print("5 layer x : "+str(x[:1]))

        x = self.activation_dropout_module(x)
        x = self.fc2(x)
        x = self.dropout_module(x)
        # print("6 layer x : "+str(x[:1]))
