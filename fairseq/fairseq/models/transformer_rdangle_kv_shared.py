...
@register_model("transformer_rdangle_kv_shared")
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if args.decoder_layerdrop > 0.0:
            self.shared_encoder_layers = LayerDropModuleList(p=args.decoder_layerdrop)
        else:
            self.shared_encoder_layers = nn.ModuleList([])
        self.shared_encoder_layers.extend(
            [
                TransformerEncoderLayer(args)
                for _ in range(args.shared_encoder_layers)
            ]
        )


        if self.decoder_layerdrop > 0.0:
            self.dis_encoder_layers = LayerDropModuleList(p=self.decoder_layerdrop)
        else:
            self.dis_encoder_layers = nn.ModuleList([])
        self.dis_encoder_layers.extend(
            [
        ...
        if args.decoder_layerdrop > 0.0:
            self.encoder_layers = LayerDropModuleList(p=args.decoder_layerdrop)
        else:
        ...
        if args.encoder_normalize_before and len(self.shared_encoder_layers) > 0:
    ...
    def extract_features_scriptable(
        ...
        #T x B x C
        for idx, layer in enumerate(self.shared_encoder_layers):
        ...
        if incremental_state is None:
            ...
            for subbatch in subbatches:
                ...
                for idx, layer in enumerate(self.dis_encoder_layers):
                    stack_dis_x = layer(stack_dis_x, stack_dis_padding_mask, part1_len = src_T, part2_len = stack_dis_x.size(0)-src_T)

                # T x B x C
                stack_dis_source_x = stack_dis_x[:src_T]
                for idx, layer in enumerate(self.encoder_layers):
                    stack_dis_source_x = layer(stack_dis_source_x, stack_source_padding_mask)

                if self.enc_layer_norm is not None:
                    stack_dis_source_x = self.enc_layer_norm(stack_dis_source_x)
                ...
                for idx, layer in enumerate(self.decoder_layers):
                    stack_target_x, _, _ = layer(
                        stack_target_x,
                        encoder_out = stack_dis_source_x,
            ...
            if (T - 1) % self.chunk_size == 0: 
                ...
                # T x B x C
                dis_source_x = dis_x[:src_T]
                for idx, layer in enumerate(self.encoder_layers):
                    dis_source_x = layer(dis_source_x, source_padding_mask)

                if self.enc_layer_norm is not None:
                    dis_source_x = self.enc_layer_norm(dis_source_x)
...
@register_model_architecture("transformer_rdangle_kv_shared", "transformer_rdangle_kv_shared")
