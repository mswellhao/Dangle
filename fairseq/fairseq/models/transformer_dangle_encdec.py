...
@register_model("transformer_dangle_encdec")
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if self.decoder_layerdrop > 0.0:
            self.dis_layers = LayerDropModuleList(p=self.decoder_layerdrop)
        else:
            self.dis_layers = nn.ModuleList([])
        self.dis_layers.extend(
            [
    ...
    def extract_features_scriptable(
        ...
        # flip positions to adapt positions to the current prediction
        ...
        if incremental_state is None:
            ...
            for i in range(1,T+1):
                ...
                target_padding_mask_i = padding_mask[:,:i]
                
                ##
                ##compute adaptive source reps
                # B x i+1 x C
                dis_x_i = torch.cat([x_i, end_token_embed], dim=1)
                # B x i+1 x C -> i+1 x B x C
                dis_x_i = dis_x_i.transpose(0, 1)
                dis_padding_mask_i = torch.cat([target_padding_mask_i, torch.zeros_like(padding_mask[:,:1])], dim=1)
                if self.target_horizon > 0:
                    dis_x_i = dis_x_i[-(self.target_horizon+1):]
                    dis_padding_mask_i = dis_padding_mask_i[:,-(self.target_horizon+1):]

                #compute adaptive source reps
                tgt_T = dis_x_i.size(0)
                # (T+i+1) x B x C
                dis_x_i = torch.cat([source_x, dis_x_i], dim=0)
                # B x (T+i+1)
                encoder_padding_mask_i = torch.cat([source_padding_mask, dis_padding_mask_i], dim=1)
                for idx, layer in enumerate(self.dis_layers):
                    # (T+i+1) x B x C
                    dis_x_i = layer(dis_x_i, encoder_padding_mask_i, part1_len = src_T, part2_len = tgt_T)
                
                # T x B x C
                source_x_i = dis_x_i[:src_T]
                for idx, layer in enumerate(self.layers):
                    source_x_i = layer(source_x_i, source_padding_mask)

                ##
                ##decoding
                # B x i x C -> i x B x C
                x_i = x_i.transpose(0, 1)
                self_attn_mask = self.buffered_future_mask(x_i)
                for idx, layer in enumerate(self.decoder_layers):
                    #i x B x C
                    x_i, _, _ = layer(
                        x_i,
                        encoder_out = source_x_i,
                        encoder_padding_mask = source_padding_mask,
                        incremental_state = None,
                        self_attn_mask=self_attn_mask,
                        self_attn_padding_mask=target_padding_mask_i,
                        need_attn=False,
            ...
            ##compute adaptive source reps
            # B x T+1 x C
            dis_x = torch.cat([x, end_token_embed], dim=1)
            # B x T+1 x C -> T+1 x B x C
            dis_x = dis_x.transpose(0, 1)
            dis_padding_mask = torch.cat([target_padding_mask, torch.zeros_like(padding_mask[:,:1])], dim=1)
            if self.target_horizon > 0:
                dis_x = dis_x[-(self.target_horizon+1):]
                dis_padding_mask = dis_padding_mask[:,-(self.target_horizon+1):]
            ...
            # B x (T+T+1)
            encoder_padding_mask = torch.cat([source_padding_mask, dis_padding_mask], dim=1)
            for idx, layer in enumerate(self.dis_layers):
                # (T+T+1) x B x C
                dis_x = layer(dis_x, encoder_padding_mask, part1_len = src_T, part2_len = tgt_T)
            
            # T x B x C
            source_x = dis_x[:src_T]
            for idx, layer in enumerate(self.layers):
...
@register_model_architecture("transformer_dangle_encdec", "transformer_dangle_encdec")
...
@register_model_architecture("transformer_dangle_encdec", "transformer_dangle_encdec_base")
...
@register_model_architecture("transformer_dangle_encdec", "transformer_dangle_encdec_base_rel_pos")
def transformer_parsing(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1024)
    args.dangle_encoder_attention_heads = getattr(args, "dangle_encoder_attention_heads", 8)
